...
- **MapEmbed** — `embedUrl: string`, `title: string`. The only place a raw third-party iframe is allowed (docs/SECURITY.md). Its origin must be present in `BaseLayout`'s CSP `frame-src`.
