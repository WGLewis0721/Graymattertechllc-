...
| `listing-1.png` through `listing-6.png` | ListingCard grid (Listings section) | 1200x900 | 4:3, exterior shot, well-lit, no people |
| `neighborhood-1.png` through `neighborhood-4.png` | NeighborhoodStrip | 1200x800 | 3:2, a representative street or landmark shot per neighborhood |
...
This vertical is image-heavy by design (listings are the hero of the page), so it needs more photo slots than most templates in this library. Minimum 4 listings, more is fine, just add matching `listings` entries in content.json and matching files here. Same for `neighborhoods` (minimum 3) and `gallery` (minimum 4, from the base schema).
